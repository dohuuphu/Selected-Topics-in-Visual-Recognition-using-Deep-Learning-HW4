...
class PromptTrainDataset(Dataset):
    ...
    def _init_all_ids(self):
        temp_ids = []
        al = self.args.data_file_dir + "all/allTrain.txt"
        temp_ids+= [id_.strip() for id_ in open(al)]
        self.al_ids = [{"clean_id":x,"de_type":3} for x in temp_ids]
        self.al_ids = self.al_ids * 120

        self.al_counter = 0
        self.num_al = len(self.al_ids)
        print("Total ALL Ids : {}".format(self.num_al))


    def _init_snow_ids(self):
        temp_ids = []
        sn = self.args.data_file_dir + "snow/snowTrain.txt"
        # temp_ids+= [self.args.derain_dir + id_.strip() for id_ in open(rs)] #/mnt/HDD1/phudh/course/selected_DL/HW3/dataset/train/rain_degraded/rain-1005.png
        temp_ids+= [id_.strip() for id_ in open(rs)] #/mnt/HDD1/phudh/course/selected_DL/HW3/dataset/train/rain_degraded/rain-1005.png

        self.sn_ids = [{"clean_id":x,"de_type":3} for x in temp_ids]
        # self.sn_ids = self.sn_ids * 120

        self.sn_counter = 0
        self.num_sn = len(self.sn_ids)
        print("Total Snow Ids : {}".format(self.num_sn))
    ...
    def _get_gt_name(self, rainy_name):
        gt_name = rainy_name.replace('/rain_degraded','/rain_clean').replace('rain-','rain_clean-')
        gt_name = gt_name.replace('/snow_degraded','/snow_clean').replace('snow-','snow_clean-')
        gt_name = gt_name.replace('/degraded','/clean')
